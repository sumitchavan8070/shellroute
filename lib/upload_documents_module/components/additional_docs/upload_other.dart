...
class UploadOther extends StatefulWidget {
  const UploadOther({
    super.key,
    required this.title,
  });
  ...
  @override
  State<UploadOther> createState() => _UploadOtherState();
}
...
class _UploadOtherState extends State<UploadOther> {
          ...
          title: Row(
            children: [
              ...
              const Text(
                'Upload Other',
                style:
                    TextStyle(color: AppColors.primary, fontWeight: FontWeight.w500, fontSize: 18),
              )
            ],
          ),
        ),
            ...
            Column(
              children: [
                ...
                const Text(
                  ...
                  style:
                      TextStyle(fontSize: 16, fontWeight: FontWeight.w500, color: AppColors.black),
                ),
                const HeightBox(10),
                const Text(
                  "Supported formats: docx, dox, xlsx, pdf, jpeg, jpg, png, txt",
                  style: TextStyle(
                      fontSize: 13, fontWeight: FontWeight.w400, color: AppColors.subtitle),
                ),
                const HeightBox(10),
                const Text(
                  "Size Limit: 10MB",
                          ...
                          children: [
                                ...
                                SizedBox(
                                  child: Align(
                                    ...
                                    child: SizedBox(
                                      height: 18,
                                      width: 15,
                                      child: SvgPicture.asset('assets/images/fileremove.svg'),
                                    ),
                                  ),
                                ).onTap(
                                  () {
                                    // widget.mapName?[indexx] =
                            ...
                            Container(
                              ...
                              child: Row(
                                ...
                                () async {
                                  controller.localFile[0] = await pickFile(context: context);
                                  debugPrint(
                                      "this is from the repallce fiel ${controller.localFile[0]}");
                              ...
                              onPressed: () {
                                if (controller.isSelected.value) {
                                  if (controller.localFile[0].path != null) {
                                    int? filesize = controller.localFile[0].size;
                                    if (filesize < (10 * 1024 * 1024) && filesize != 0) {
                                      context.pushNamed(GoPath.uploadOtherConfirmation,
                                          pathParameters: {
                                            'title': widget.title.toString(),
                                            'title2': "1 file added"
                                          },
                                          extra: controller.additional);
                                      // _getXController.isSelected.value = false;
                                    } else {
                                      snackbar(context, 'fileSize should be less than 10MB!!');
                                      controller.isSelected.value = false;
                                    }
                                  } else {
                                    controller.isSelected.value = false;
